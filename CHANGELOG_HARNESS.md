...
## 2026-08-31：统一公考晨读生产标准定版

- 新增 content_harness/gongkao_morning_reading_production_standard.md，作为选文、生成、质检、定点重写和最终发布的唯一有效标准。
- 正式统一四项长期口径：答题框架 3—4 点且每点不超过 45 字；情景实务题必须有身份、纯申论分析题不强制身份；今日可带走固定为“关键词＋常识点＋两句表达＋方法框架”；完整版移动端阅读时长为 5—8 分钟。
- 同步补充选文舍弃条件、语义去重、时间节点仅加分、原文忠实拆解、机关权限边界、弱政策匹配隐藏、CTA 成品约束、质检误报复核和最终成品全链路同步。
- 历史模块文档如与新标准冲突，以新标准为准；本次仅统一文档口径，未改变运行时代码或门禁阈值。


## 2026-08-31：每日晨读生成技能骨架
