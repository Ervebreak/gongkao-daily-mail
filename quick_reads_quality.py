...
EXAM_VALUE_KEYWORDS = [
    "考点", "申论", "面试", "公基", "题型", "答题", "作答", "素材", "案例", "表达", "表述", "金句",
    "框架", "对策", "综合分析", "贯彻执行", "基层治理", "公共服务", "民生", "治理", "政策落实", "热点背景", "积累",
]
NEWS_SUMMARY_MARKERS = ["报道了", "介绍了", "讲述了", "指出了", "强调了", "展示了", "反映了", "体现了", "本文主要", "文章主要", "新闻主要"]
GENERIC_VALUE_PHRASES = ["值得关注", "有一定价值", "可以了解", "有参考意义", "拓展视野", "补充阅读", "学习了解"]
...
DANGLING_ENDINGS = (
    ...
    "建立", "转向", "转为", "赋能", "配套", "提出从", "让", "把", "与", "和", "及", "并", "但", "而", "在", "为", "的", "监",
)
...
def _quick_reason(item: Any) -> str:
    if isinstance(item, dict):
        return _text(item.get("reason") or item.get("exam_value") or item.get("usable_for_exam") or item.get("value") or item.get("summary"))
    return _text(item)
...
def _keyword_hits(text: str, keywords: list[str] | tuple[str, ...]) -> list[str]:
...
def _classify_url_issue(idx: int, item: dict[str, Any]) -> dict[str, str] | None:
    ...
    if is_ssl_warning and has_fallback:
        return {"severity": "low", "code": "quick_read_url_ssl_warning", "message": f"第{idx}篇速读链接 SSL 校验警告：{url_reason}；已有标题/来源可作为备用搜索，不阻断发送。"}
    if has_fallback:
        return {"severity": "medium", "code": "quick_read_url_warning", "message": f"第{idx}篇速读链接状态异常：{url_status} {url_reason}；已有标题/来源可备用搜索。"}
    return {"severity": "high", "code": "quick_read_url_not_valid", "message": f"第{idx}篇速读链接状态异常且缺少备用搜索信息：{url_status} {url_reason}"}


def evaluate_quick_reads(brief: dict[str, Any]) -> dict[str, Any]:
    quick_reads = _as_list(brief.get("quick_reads"))
    issues: list[dict[str, str]] = []
    checks = {"quick_read_count": len(quick_reads), "items_with_exam_value": 0, "items_looking_like_news_summary": 0, "items_missing_reason": 0}
    ...
    for idx, item in enumerate(quick_reads[:3], start=1):
        ...
        combined = f"{title} {reason} {one_sentence}"
