...
def evaluate_takeaway(brief: dict[str, Any]) -> dict[str, Any]:
    ...
    if not common_points:
        ...
        issues.append({"severity": "high", "code": "truncated_common_knowledge", "message": "时政常识点疑似半句话，需要改成完整说明"})
    ...
    for idx, item in enumerate(golden[:3], start=1):
        ...
        if _looks_incomplete(sentence):
            issues.append({"severity": "high", "code": "truncated_takeaway", "message": f"第{idx}条金句存在省略号或疑似半句话，需要重写为更短的完整句"})
        if len(sentence) > 45:
        ...
        if not scenario or scenario in GENERIC_SCENARIOS:
            ...
            issues.append({"severity": "high", "code": "truncated_golden_usage", "message": f"第{idx}条金句适用场景疑似半句话，需要改成更短的完整说明"})

    if not framework:
        issues.append({"severity": "medium", "code": "missing_framework", "message": "缺少可迁移框架"})
    elif _looks_incomplete(framework):
        issues.append({"severity": "high", "code": "truncated_takeaway", "message": "可迁移框架存在省略号或疑似截断"})
