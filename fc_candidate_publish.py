...
def _blocked_response(error: DailyPublishError) -> dict[str, Any]:
    return _json_response(
        error.http_status,
        {"status": "blocked", "issue_code": error.code, "message": error.message},
    )
...
def _authorized(event: Any) -> bool:
    ...
    return bool(separator and scheme.lower() == "bearer" and supplied and hmac.compare_digest(supplied, expected))
...
def _request_payload(event: Any) -> dict[str, Any]:
    ...
    if missing or unknown:
        ...
        raise DailyPublishError("daily_candidate_input_invalid", "; ".join(details))
    return raw
...
def _validate_quality_state(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(nested_gate, dict):
        raise DailyPublishError("daily_candidate_quality_state_drift", "candidate.quality.gate is missing or invalid.")
    if top_gate != nested_gate:
        raise DailyPublishError("daily_candidate_quality_state_drift", "candidate.quality_gate does not match candidate.quality.gate.")
    ...
    if gate.get("overall") != "ok" or int(gate.get("p0_count") or 0) != 0:
        raise DailyPublishError("daily_candidate_quality_blocked", "Candidate quality gate is not publishable.")
    ...
    if not isinstance(consistency, dict) or consistency.get("quality_gate_synced") is not True:
        raise DailyPublishError("daily_candidate_quality_state_drift", "Candidate quality gate state is not synchronized.")
    return normalized
...
def _validate_source_evidence(candidate: dict[str, Any], brief: dict[str, Any]) -> None:
    ...
    if not isinstance(top, dict) or not isinstance(embedded, dict) or not top or not embedded:
        raise DailyPublishError("daily_candidate_source_evidence_invalid", "Verified source evidence is required.")
    if top != embedded:
        raise DailyPublishError("daily_candidate_source_evidence_invalid", "Top-level source_evidence does not match brief._source_evidence.")
    items = top.get("items")
    if not isinstance(items, dict) or not items:
        raise DailyPublishError("daily_candidate_source_evidence_invalid", "Source evidence contains no verified articles.")
    if top.get("all_verified") is not True:
        raise DailyPublishError("daily_candidate_source_evidence_invalid", "Source evidence is not fully verified.")
    if any(not isinstance(item, dict) or item.get("verification_status") != "verified" for item in items.values()):
        raise DailyPublishError("daily_candidate_source_evidence_invalid", "Every source evidence item must be verified.")
...
def _validate_fact_review(candidate: dict[str, Any], brief: dict[str, Any]) -> None:
    ...
    if not isinstance(top, dict) or not isinstance(embedded, dict) or not top or not embedded:
        raise DailyPublishError("daily_candidate_fact_review_invalid", "A completed fact review is required.")
    if top != embedded:
        raise DailyPublishError("daily_candidate_fact_review_stale", "Top-level fact_review does not match brief._fact_review.")
    if top.get("ok") is not True or top.get("status") != "ok":
        raise DailyPublishError("daily_candidate_fact_review_invalid", "Fact review is not in final ok state.")
    if not fact_review_binding_is_current(brief, top):
        raise DailyPublishError("daily_candidate_fact_review_stale", "Fact review binding does not match the submitted candidate.")
...
def _authoritative_selection_featured(brief: dict[str, Any]) -> dict[str, Any]:
    ...
    if not featured:
        raise DailyPublishError("daily_candidate_selection_score_invalid", "Authoritative featured selection metadata is missing.")
    return featured
...
def _validate_selection_score(brief: dict[str, Any]) -> None:
    ...
    if not isinstance(detail, dict):
        raise DailyPublishError("daily_candidate_selection_score_invalid", "Featured selection score_detail is missing.")
    computed = 0
    for key, maximum in EXPECTED_SCORE_DETAIL.items():
        if key not in detail:
            raise DailyPublishError("daily_candidate_selection_score_invalid", f"Featured selection score_detail is missing {key}.")
        value = _score_int(detail.get(key), f"score_detail.{key}")
        if value < 0 or value > maximum:
            raise DailyPublishError("daily_candidate_selection_score_invalid", f"score_detail.{key} is outside the allowed range.")
        computed += value
    total = _score_int(featured.get("total_score"), "total_score")
    if total != computed:
        raise DailyPublishError("daily_candidate_selection_score_invalid", "Featured selection total_score does not equal the six score dimensions.")
    if total < 75:
        raise DailyPublishError("daily_candidate_selection_score_invalid", "Featured selection total_score is below the publish threshold.")


def _validate_renderability(candidate: dict[str, Any], brief: dict[str, Any]) -> None:
    # Keep snapshots in the candidate schema, but the canonical state is `brief`.
    # Publisher only proves that the current production renderer can render that
    # brief; byte-for-byte snapshot equality belongs to the Skill generation QA.
    for key in ("plain_text", "html_body"):
        if not isinstance(candidate.get(key), str):
            raise DailyPublishError("daily_candidate_input_invalid", f"candidate.{key} must be a string.")
    try:
        rendered_plain = render_plain_text(brief)
        rendered_html = render_email_html(brief)
    except Exception as exc:
        raise DailyPublishError("daily_candidate_render_invalid", "Final brief could not be rendered by the current production renderer.") from exc
    if not isinstance(rendered_plain, str) or not rendered_plain.strip():
        raise DailyPublishError("daily_candidate_render_invalid", "Current production plain-text renderer returned empty output.")
    if not isinstance(rendered_html, str) or not rendered_html.strip():
        raise DailyPublishError("daily_candidate_render_invalid", "Current production HTML renderer returned empty output.")
...
def _validate_candidate(candidate: Any) -> dict[str, Any]:
    ...
    if delivery_date != brief_date:
        raise DailyPublishError("daily_candidate_date_mismatch", "candidate.delivery_date does not match candidate.brief.date.")
    ...
    _validate_renderability(normalized, brief)
    return normalized


def _read_latest_from_oss_strict() -> dict[str, Any] | None:
    # _get_oss_json distinguishes a true 404 from other OSS failures in meta.
    # Do not use load_candidate() here because its local/bundled fallback would
    # make an OSS read failure indistinguishable from "latest does not exist".
    try:
        latest, meta = candidate_store._get_oss_json(candidate_store.latest_object_key())
    except Exception as exc:
    ...
    if latest is not None:
        if meta.get("candidate_oss_read_ok") is not True:
            raise DailyPublishError(
                "daily_candidate_stale_check_failed",
                "Current latest candidate read returned an invalid state.",
                http_status=503,
            )
        return latest
    error = str(meta.get("candidate_oss_error") or "")
...
def _validate_not_stale(candidate: dict[str, Any]) -> None:
    ...
    latest = _read_latest_from_oss_strict()
    if latest is None:
        return
    ...
    try:
        ...
        raise DailyPublishError(
            "daily_candidate_stale_check_failed",
            "Current latest candidate has an invalid delivery_date.",
            http_status=503,
        ) from exc
    if incoming < current:
        raise DailyPublishError(
...
def _publish(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise DailyPublishError("daily_candidate_save_failed", "Candidate could not be saved.", http_status=502) from exc
    if result.get("candidate_saved") is not True:
        raise DailyPublishError("daily_candidate_save_failed", "Candidate save did not complete.", http_status=502)
    if result.get("candidate_oss_write_ok") is not True:
        raise DailyPublishError("daily_candidate_oss_write_failed", "Daily candidate OSS write failed.", http_status=502)
    if result.get("candidate_oss_latest_write_ok") is not True:
        raise DailyPublishError("daily_candidate_latest_write_failed", "Latest candidate OSS write failed.", http_status=502)
    ...
    return {
        ...
        "daily_path": str(
            result.get("candidate_oss_path")
            or f"oss://{settings.oss_bucket}/{candidate_store.candidate_object_key(delivery_date)}"
        ),
...
def handler(event: Any, context: Any) -> dict[str, Any]:
    ...
    if not _authorized(envelope):
        return _blocked_response(DailyPublishError("daily_publish_auth_failed", "Unauthorized.", http_status=401))
    try:
        payload = _request_payload(envelope)
        candidate = _validate_candidate(payload["candidate"])
        _validate_not_stale(candidate)
        return _json_response(200, _publish(candidate))
    except DailyPublishError as exc:
        return _blocked_response(exc)
    except Exception:
        return _blocked_response(
            DailyPublishError("daily_candidate_publish_failed", "Candidate publish failed.", http_status=500)
        )
